...
url_runmut = "https://mutalyzer.nl/json/runMutalyzer?variant=NG_009060.1(LDLR):"
...
war_msg <- ifelse(mut_output$errors == 0 & mut_output$warnings > 0,mut_output$messages[[1]]$message,"NA")
  ...
  if (mut_output$errors == 0 & (mut_output$warnings == 0 | (mut_output$warnings > 0 & !str_detect(war_msg,"is identical to the variant|No mutation given")))) {
      ...
      NG_009060.1 <- mut_output$original
      ...
      met_any <- ifelse((met_A_acc == 1 | met_A_don == 1 | met_B_acc == 1 | met_A_don == 1),1,0)
      ...
      if(met_any == 1) {
        ...
        if (affecting_exon %in% c(2:17)) {
          wt_acc_motif = substring(NG_009060.1,exon_start-20,exon_start+2)
          wt_acc_motif_score = as.numeric(str_extract(content(GET(paste0(acc_part1,wt_acc_motif,acc_part2)), "text"), "(?<=(MAXENT: )).?\\d{1,2}.\\d{1,2}"))
          wt_don_motif = substring(NG_009060.1,exon_end-2,exon_end+6)
        ...
        if(met_A_acc == 1){
          if(ref_end < exon_start) {
            A_alt_ag_motif = paste0(substring(NG_009060.1,(exon_start-20)-(nchar_ref - nchar_alt),ref_start-1),ALT,substring(NG_009060.1,ref_end+1,(exon_start+2)))
          }else if(ref_start == exon_start) {
            A_alt_ag_motif = paste0(substring(NG_009060.1,(exon_start-20),(exon_start-20)+19),substring((paste0(ALT,substring(NG_009060.1,ref_end+1,ref_end+3))),1,3)) 
          }else if(ref_start > exon_start) {
            A_alt_ag_motif = paste0(substring(NG_009060.1,(exon_start-20),(exon_start-20)+19),substring((paste0(substring(NG_009060.1,exon_start,ref_start-1),ALT,substring(NG_009060.1,ref_end+1,ref_end+3))),1,3)) 
          }
        }else if(met_A_acc == 0){
          A_alt_ag_motif <- NA
        }
        ...
        if(met_A_don == 1){
          if(ref_start > exon_end) {
            A_alt_gt_motif = paste0(substring(NG_009060.1,(exon_end-2),ref_start-1),ALT,substring(NG_009060.1,ref_end+1,((exon_end+6) + (nchar_ref - nchar_alt))))
          }else if(ref_end <= exon_end) {
            up_stream = paste0(substring(NG_009060.1,ref_start-3,ref_start-1),ALT,substring(NG_009060.1,ref_end+1,exon_end))
            A_alt_gt_motif = paste0(substring(up_stream,nchar(up_stream)-2,nchar(up_stream)),substring(NG_009060.1,exon_end+1,exon_end+6))
          }
        }else if(met_A_don == 0){
          A_alt_gt_motif <- NA
        }
        ...
        B_ref_ag_seq = case_when(met_B_acc == 1 ~ substring(NG_009060.1,ref_start-1,ref_end+1))
        B_alt_ag_seq = case_when(met_B_acc == 1 ~ paste0(substring(NG_009060.1,ref_start-1,ref_start-1),ALT,substring(NG_009060.1,ref_end+1,ref_end+1)))
        ...
        if(!is.na(new_AG) & new_AG == 1){
          ...
          genome_up = substring(NG_009060.1,ref_start-19,ref_start-2)
          ...
          genome_down = substring(NG_009060.1,ref_end+2,ref_end+4)
        ...
        B_ref_gt_seq = case_when(met_B_don == 1 ~ substring(NG_009060.1,ref_start-1,ref_end+1))
        B_alt_gt_seq = case_when(met_B_don == 1 ~ paste0(substring(NG_009060.1,ref_start-1,ref_start-1),ALT,substring(NG_009060.1,ref_end+1,ref_end+1)))
        ...
        if(!is.na(new_GT) & new_GT == 1){
          ...
          genome_up = substring(NG_009060.1,ref_start-4,ref_start-2)
          alt_up = substring(alt_seq,1,ag_start-1)
          all_up = paste0(genome_up,alt_up)
          all_up_len = nchar(all_up)
          up_section = ifelse(ag_start == 1,genome_up,substring(all_up,all_up_len-2,all_up_len))
          alt_down = substring(alt_seq,ag_end+1,alt_len)
          genome_down = substring(NG_009060.1,ref_end+2,ref_end+5)
          all_down = paste0(alt_down,genome_down)
          down_section = substring(all_down,1,4)
          B_alt_gt_motif = paste0(up_section,"GT",down_section)
        }else{B_alt_gt_motif = NA}
        
        if(!is.na(new_GT) & new_GT == 1){rm(ag_end,ag_start,all_down,all_up_len,all_up,alt_down,alt_len,alt_seq,alt_up,down_section,genome_down,genome_up,i,up_section)}
        ...
        if (met_B_acc == 1) {
          C_down_stream_ag = paste0(substring(ALT,nchar_alt,nchar_alt),substring(NG_009060.1,ref_end+1,(ref_end+1+min(18,((exon_end-48)-(ref_end+1))))))
        }else {
          C_down_stream_ag <- NA}
        ...
        if (met_B_acc == 1) {
          C_up_stream_ag = paste0(substring(NG_009060.1,max(ref_start-4,exon_start),ref_start-1),substring(ALT,1,1))
        }else {
          C_up_stream_ag <- NA
        }
        ...
        if (!is.na(C_AG_down) & C_AG_down == 1 & ((!is.na(new_AG) & new_AG == 0) | is.na(new_AG))) {
          ...
          C_wt_acc_cryptic_down <- substring(NG_009060.1,(ref_end+(G-1))-19,(ref_end+(G-1))+3)
          first = substring(NG_009060.1,((ref_end+(G-1))-19)+(nchar_alt-nchar_ref),ref_start-1)
          last = substring(NG_009060.1,ref_end+1,(ref_end+(G-1))+3)
        ...
        if (!is.na(C_AG_up) & C_AG_up == 1 & ((!is.na(new_AG) & new_AG == 0) | is.na(new_AG))) {
          ...
          C_wt_acc_cryptic_up <- substring(NG_009060.1,(ref_start-G)-19,((ref_start)-G)+3)
          first = substring(NG_009060.1,(ref_start-G)-19,ref_start-1)
          last = substring(NG_009060.1,ref_end+1,ref_end+3)
        ...
        if (met_B_don == 1) {
          C_down_stream_gt = paste0(substring(ALT,nchar_alt,nchar_alt),substring(NG_009060.1,ref_end+1,(ref_end+1+min(3,(exon_end-(ref_end+1))))))
        }else {
          C_down_stream_gt <- NA
        }
        ...
        if (met_B_don == 1) {
          C_up_stream_gt = paste0(substring(NG_009060.1,max(ref_start-5,exon_start+48),ref_start-1),substring(ALT,1,1))
        }else {
          C_up_stream_gt <- NA
        }
        ...
        if (!is.na(C_GT_down) & C_GT_down == 1 & ((!is.na(new_GT) & new_GT == 0) | is.na(new_GT))) {
          ...
          C_wt_don_cryptic_down <- substring(NG_009060.1,(ref_end+(G-1))-3,(ref_end+(G-1))+5)
          first = substring(NG_009060.1,((ref_end+(G-1))-3)+(nchar_alt-nchar_ref),ref_start-1)
          last = substring(NG_009060.1,ref_end+1,(ref_end+(G-1))+5)
        ...
        if (!is.na(C_GT_up) & C_GT_up == 1 & ((!is.na(new_GT) & new_GT == 0) | is.na(new_GT))) {
          ...
          C_wt_don_cryptic_up <- substring(NG_009060.1,(ref_start-G)-3,((ref_start)-G)+5)
          first = substring(NG_009060.1,(ref_start-G)-3,ref_start-1)
          last = substring(NG_009060.1,ref_end+1,ref_end+5)
